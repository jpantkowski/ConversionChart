...
jednocześnie nie wywołuje zapytania dla każdej daty. Zastosowano paging po 365 dat wiec program wywołuje zapytanie
...
W rezultacie otrzymujemy listę, której każdy element jest słownikiem którego kluczami są: Currency, Code i Mid.
...
3. Wywołujemy pętlę - jeżeli funkcja are_dates_valid zwraca False, znaczy to, że coś jest nie tak z datą. 
...
c. czy data końcowa nie wypada w przyszłości.
...
def dates_input(currency_CODE):
    ...
    match currency:
        case "USD" | "AUD" | "CAD" | "EUR" | "HUF" | "CHF" | "GBP" | "JPY" | "CZK" | "DKK" | "NOK" | "SEK" | "XDR":
            API_start_date = "2002-01-02"
        case "HKD" | "UAH" | "ZAR":
            API_start_date = "2003-01-07"
        case "RON" | "BGN":
            API_start_date = "2007-04-04"
        case "THB" | "NZD" | "SGD" | "ISK" | "TRY" | "PHP" | "MXN" | "BRL" | "MYR" | "IDR" | "KRW" | "CNY":
    ...
    if parse(start_date) < parse(API_start_date):
        ...
        print(f"Kursy dostępne od {API_start_date}! Data początkowa zmieniona na {API_start_date}")
    while are_dates_valid(start_date, end_date) != True:
        return dates_input()
    dates_range = []
    if parse(start_date) < parse("2002-01-02"):
...
7. Funkcja zwraca listę, której każdy element to słownik odpowiadający za daną datę i posiadający kurs danej waluty."""
...
def print_chart():
    ...
    if len(rate_dict)==0:
        ...
        if not max_rate == min_rate:
            ...
            plt.text(x = df.x.iloc[max_index], y = max_rate, s = f"MAX {max_rate} - {df.x[max_index]}")
            plt.text(x = df.x.iloc[min_index], y = min_rate, s = f"MIN {min_rate} - {df.x[min_index]}")
